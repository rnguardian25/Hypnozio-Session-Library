...
function buildLangOpts() {
  ...
  LANGUAGES.forEach(l => {
    ...
    div.setAttribute('data-code', l.code);
    div.innerHTML = `
...
function closeLangPopover() {
  ...
  document.getElementById('lang-caret').style.transform = '';
}

document.addEventListener('click', (e) => {
  if (!document.getElementById('lang-wrap').contains(e.target)) closeLangPopover();
  // close any open card info popups
...
function switchLang(code) {
  ...
  document.getElementById('lang-current-label').textContent = found ? found.label : code;
  render();
}
...
function render() {
  ...
  const progs = s.programs[lang] || s.programs['en']; // fallback to EN
  ...
  document.getElementById('h-sub').innerHTML =
    `${progs.length} audio programs &nbsp;·&nbsp; <strong>${langLabel}</strong>`;
...
function toggleInfoPopup(e, btn) {
  e.stopPropagation();
  const popup = btn.nextElementSibling; // .card-info-popup
